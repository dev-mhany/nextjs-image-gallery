...
def copy_image(image, id, labelID, montage=False):
    if montage:
        filename = "Face ID #{}".format(labelID)
...
if __name__ == "__main__":
    ...
    for labelID in labelIDs:
        ...
        for i in idxs:
            ...
            key = os.path.basename(data[i]["imagePath"])
            prefix = key.replace('.', '_')
            
            if prefix not in counter:
                counter[prefix] = 0
            facename = prefix + '_' + str(counter[prefix])
            counter[prefix] += 1
            # force resize the face ROI to 96mx96 and then add it to the
            # faces montage list
            face = cv2.resize(face, (32, 32))
            # putting the image in the clustered folder
            copy_image(face, facename, "faces")
            faces.append(face)
            
            if(str(labelID) != '-1'):
                if str(labelID) not in faceData:
                    faceData[str(labelID)] = []
                faceData[str(labelID)].append(key)
            
                # add faces id to image key
                if key not in imagesData:
                    imagesData[key] = []
                newImage = {'k':int(labelID),'n': facename}
                imagesData[key].append(newImage)

        # create a montage using 96x96 "tiles" with 5 rows and 5 columns
        if(labelID == -1):
            montage = build_montages(faces, (96, 96), (30, 30))[0]
        else:
            montage = build_montages(faces, (96, 96), (6, 6))[0]
